...
window.AppKeuanganAkuntansi = {
    ...
    init: function() {
        ...
        Promise.all([pTrx, pKasKeluar, pBeliStok, pGaji, pJurnalManual, pSaldoAwal]).then(function(results) {
            ...
            results[2].forEach(function(doc) {
                ...
                if (b.items) {
                    b.items.forEach(function(it) {
                        if (it.isPPN) valPPNGross += (it.qty * it.hargaBeli);
                        else valNonPPN += (it.qty * it.hargaBeli);
                    });
                }
            ...
            // FIX: skema payrollHistory adalah 1 dokumen per karyawan (field: bulan, totalGaji),
            //      bukan 1 dokumen per bulan dengan array dataPenggajian. Jumlahkan totalGaji semua
            //      karyawan pada bulan terpilih lalu buat 1 pasang jurnal gaji+kas keluar.
            var totalGajiBulan = 0;
            results[3].forEach(function(doc) {
                var g = doc.data();
                totalGajiBulan += g.totalGaji || 0;
            });
            if (totalGajiBulan > 0) {
                var tglGaji = bulan + '-01';
                self.dataJurnal.push({ tanggal: tglGaji, keterangan: 'Beban Gaji & Tunjangan Karyawan', akunDebit: '5-2100', akunKredit: '', debit: totalGajiBulan, kredit: 0, isManual: false, tipeJurnal: 'Otomatis' });
                self.dataJurnal.push({ tanggal: tglGaji, keterangan: 'Kas Keluar Payroll', akunDebit: '', akunKredit: '1-1100', debit: 0, kredit: totalGajiBulan, isManual: false, tipeJurnal: 'Otomatis' });
            }
