...
window.AppApotekObat = {
    ...
    openForm: function(id) {
        ...
        html += '<div class="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-900/30 rounded-lg">';
        html += '<input type="checkbox" id="fo-ppn" ' + (o.isPPN ? 'checked' : '') + ' class="w-4 h-4 rounded border-slate-300 text-primary-600 focus:ring-primary-500">';
        html += '<label for="fo-ppn" class="text-sm text-slate-700 dark:text-slate-300">Obat Kena PPN (OTC / Ethical)</label>';
        html += '</div>';
    ...
    simpan: function() {
        ...
        var obj = {
            ...
            isPPN: document.getElementById('fo-ppn').checked,
    ...
    downloadTemplate: function() {
        var ws_data = [
            ...
            ['OB-001', 'Paracetamol 500mg', 'Tablet', 'Strip', 5000, 8000, 100, 10, '2025-12-31', 'Tidak'],
        ];
        var ws = XLSX.utils.aoa_to_sheet(ws_data);
        ws['!cols'] = [{wch: 12}, {wch: 30}, {wch: 12}, {wch: 10}, {wch: 12}, {wch: 15}, {wch: 12}, {wch: 15}, {wch: 20}, {wch: 18}];
        ...
        reader.onload = function(e) {
            try {
                ...
                AppApotekObat.importData = jsonData.map(row => ({
                    ...
                    isPPN: String(row['Kena PPN (Ya/Tidak)'] || '').trim().toLowerCase() === 'ya'
