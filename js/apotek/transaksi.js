...
window.AppApotekTransaksi = {
    ...
    init: function() {
        ...
        Promise.all([
            ...
            // Resep klinik yang belum selesai (menunggu ATAU sedang diproses apoteker)
            self.resepList = [];
            results[2].forEach(function(doc) {
                var d = doc.data(); d.id = doc.id;
                if (!d.statusResep || d.statusResep !== 'selesai') self.resepList.push(d);
            });

            self.renderForm();

            // FIX: konsumsi handoff dari halaman Resep ("Proses di Kasir") supaya
            // resep yang dipilih otomatis ter-set, bukan harus dicari manual lagi.
            if (window.TEMP_RESEP_ID_FOR_KASIR) {
                var presetResepId = window.TEMP_RESEP_ID_FOR_KASIR;
                delete window.TEMP_RESEP_ID_FOR_KASIR;
                self.setTipe('resep_klinik');
                var resepSelect = document.getElementById('trx-resep-id');
                if (resepSelect) {
                    resepSelect.value = presetResepId;
                    self.onSelectResep();
                }
            }
        }).catch(function(err) {
    ...
    _renderHeader: function(tipe) {
        ...
        if (tipe === 'resep_klinik') {
            ...
            html += '<option value="">-- Pilih Resep (' + this.resepList.length + ') --</option>';
