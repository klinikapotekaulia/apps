 ...
 *   - obat  { namaObat, kodeObat, stok, stokMinimum, satuan, expDate, hpp }
 *
 * CARA PASANG:
...
window.AppApotekNotifikasi = {
    ...
    renderKadaluarsa: function() {
        ...
        // Filter obat yang punya expDate dan <= batas, stok > 0
        var list = this.allObat.filter(function(o) {
            return o.expDate && o.expDate <= batasStr && (o.stok || 0) > 0;
        }).sort(function(a, b) {
        ...
        var sudahExp = this.allObat.filter(function(o) {
            return o.expDate && o.expDate < nowStr && (o.stok || 0) > 0;
        });
    ...
    exportLaporan: function() {
        ...
        var nowStr = now.toISOString().split('T')[0];
        var expList = this.allObat.filter(function(o) { return o.expDate && o.expDate <= batas60Str && (o.stok || 0) > 0; });
        lines.push('--- KADALUARSA DALAM 60 HARI (' + expList.length + ' item) ---');
        if (expList.length === 0) {
            lines.push('  (tidak ada)');
        } else {
            expList.forEach(function(o) {
                var selisih = Math.ceil((new Date(o.expDate) - now) / (1000 * 60 * 60 * 24));
                lines.push('  [' + (o.kodeObat || '-') + '] ' + o.namaObat + ' — Exp: ' + o.expDate + ' (' + (selisih < 0 ? 'SUDAH LEWAT' : selisih + ' hari lagi') + ') Stok: ' + (o.stok || 0) + ' ' + (o.satuan || ''));
            });
        }
    ...
    loadBadge: function(callback) {
        db.collection('obat').get().then(function(snap) {
            ...
            snap.forEach(function(doc) {
                ...
                var expAlert   = d.expDate && d.expDate <= batas30Str && (d.stok || 0) > 0;
