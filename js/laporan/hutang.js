...
window.AppLaporanHutang = {
    ...
    init: function() {
        var self = this;
        // FIX: '!=' di Firestore akan mengecualikan dokumen lama yang belum punya field
        // statusPelunasan sama sekali. Pakai 'in' dengan status non-lunas yang dikenal.
        db.collection('pembelian').where('statusPelunasan', 'in', ['belum_lunas', 'menunggu_approve']).get().then(snap => {
            self.data = [];
